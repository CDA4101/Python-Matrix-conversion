...
def decrypt(image):
    ...
    cypher2d = cypher.reshape(-1, cypher.shape[1])
    c2 = cypher2d[:2628, :2628]
    i2d = img2d[:2628, :2628]

    cInv = inv(c2)

    res2d = np.dot(i2d, cInv) 
    print(res2d.shape)
    res3d = res2d.reshape(len(image), len(image[0]), 3)

    # res_res = Image.fromarray(res3d, 'RGB')
    # res_res.save('res_res.png')
